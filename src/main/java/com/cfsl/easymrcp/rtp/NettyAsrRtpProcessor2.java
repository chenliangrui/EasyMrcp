...
/**
 * ASR RTP音频处理器 - 直接PCM模式
 * 负责接收和处理ASR的RTP音频数据，直接使用L16编码，支持VAD检测和音频缓存
 * 省略G.711解码，假设TTS直接输出8kHz 16bit PCM，FreeSWITCH协商使用L16编码
 */
@Slf4j
public class NettyAsrRtpProcessor2 extends ChannelInitializer<DatagramChannel> {
    @Setter
    ...
    public NettyAsrRtpProcessor2(int mediaType) {
    ...
    /**
     * 处理接收到的RTP数据，直接返回L16编码的PCM数据
     * 省略G.711解码，直接解析RTP payload作为PCM数据
     */
    private ByteBuf processRtpData(ByteBuf rtpData, ByteBufAllocator allocator) {
        try {
            ...
            byte[] l16Data = parsedPacket.getPayload();

            // 字节序转换（网络字节序/大端到小端）
            byte[] pcmData = convertL16Endianness(l16Data);
    ...
    /**
     * 转换L16音频数据的字节序
     * RTP传输的L16是大端序，Java处理通常是小端序
     */
    private byte[] convertL16Endianness(byte[] data) {
        byte[] result = new byte[data.length];
        for (int i = 0; i < data.length; i += 2) {
            if (i + 1 < data.length) {
                result[i] = data[i + 1];   // 低字节
                result[i + 1] = data[i]; // 高字节
            }
        }
        return result;
    }
