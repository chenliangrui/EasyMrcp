...
@Slf4j
/**
 * 阿里云 FunASR 长时间转写 WebSocket 协议适配器。
 */
public class AliyunFunasrTransliterateWsClient extends WebSocketListener {
    ...
    public void onOpen(WebSocket webSocket, Response response) {
        ...
        if (!accepted) {
            ...
            releaseStartLatch();
            log.warn("阿里云 FunASR 长时间转写 run-task 发送被拒绝，taskId={}", taskId);
    ...
    public void onMessage(WebSocket webSocket, String text) {
        ...
        switch (event) {
            ...
            case "task-failed":
                releaseStartLatch();
                terminalStateReached.set(true);
                log.warn("阿里云 FunASR 长时间转写任务失败，taskId={}", taskId);
                break;
            default:
                log.debug("忽略阿里云 FunASR 长时间转写事件 {}，taskId={}", event, taskId);
                break;
        }
    }
    ...
    public void onFailure(WebSocket webSocket, Throwable t, Response response) {
        releaseStartLatch();
        terminalStateReached.set(true);
        log.error("阿里云 FunASR 长时间转写 WebSocket 异常，taskId={}", taskId, t);
        if (response != null) {
            response.close();
        }
    }
