...
public class TcpClientNotifier {
     ...
     * @return 是否发送成功
     */
    public boolean sendTtsStartNotify(String clientId) {
        if (clientId == null || clientId.isEmpty()) {
            LOGGER.warn("客户端ID为空，无法发送TTS开始播放通知");
            return false;
        }
        
        // 检查客户端是否存在
        if (!connectionManager.hasClient(clientId)) {
            LOGGER.warn("客户端不存在，无法发送TTS开始播放通知: {}", clientId);
            return false;
        }
        ...
        try {
            ...
            response.setId(clientId);
            response.setCode(200);
            response.setMessage("TTS开始");
            response.setData("TTS开始播放");
            
            return connectionManager.sendToClient(clientId, response);
        } catch (Exception e) {
            LOGGER.error("发送TTS开始播放通知异常", e);
            return false;
        }
    }
     ...
     * @return 是否发送成功
     */
    public boolean sendTtsEndNotify(String clientId) {
        if (clientId == null || clientId.isEmpty()) {
            LOGGER.warn("客户端ID为空，无法发送TTS结束播放通知");
            return false;
        }
        
        // 检查客户端是否存在
        if (!connectionManager.hasClient(clientId)) {
            LOGGER.warn("客户端不存在，无法发送TTS结束播放通知: {}", clientId);
            return false;
        }
        ...
        try {
            ...
            response.setId(clientId);
            response.setCode(200);
            response.setMessage("TTS结束");
            response.setData("TTS播放结束");
            
            return connectionManager.sendToClient(clientId, response);
        } catch (Exception e) {
            LOGGER.error("发送TTS结束播放通知异常", e);
            return false;
        }
    }
     ...
     * @return 是否发送成功
     */
    public boolean sendErrorNotify(String clientId, String errorMessage) {
        if (clientId == null || clientId.isEmpty()) {
            LOGGER.warn("客户端ID为空，无法发送错误通知");
            return false;
        }
        
        // 检查客户端是否存在
        if (!connectionManager.hasClient(clientId)) {
            LOGGER.warn("客户端不存在，无法发送错误通知: {}", clientId);
            return false;
        }
        
        try {
            TcpResponse response = TcpResponse.error(clientId, errorMessage);
            
            return connectionManager.sendToClient(clientId, response);
        } catch (Exception e) {
            LOGGER.error("发送错误通知异常", e);
            return false;
        }
    }
} 