...
public class FunasrWsClientTest extends WebSocketClient {

  public class RecWavThread extends Thread {
    private FunasrWsClientTest funasrClient;

    public RecWavThread(FunasrWsClientTest funasrClient) {
  ...
  private static final Logger logger = LoggerFactory.getLogger(FunasrWsClientTest.class);

  public FunasrWsClientTest(URI serverUri, Draft draft) {
    super(serverUri, draft);
  }

  public FunasrWsClientTest(URI serverURI) {
    
	super(serverURI);
  }

  public FunasrWsClientTest(URI serverUri, Map<String, String> httpHeaders) {
  ...
  public void sendJson(
    ...
    try {
	  ...
	  if(FunasrWsClientTest.hotwords.trim().length()>0)
	  {
		  String regex = "\\d+";
		  JSONObject jsonitems = new JSONObject();
		  String[] items= FunasrWsClientTest.hotwords.trim().split(" ");
  ...
  public void recWav() {
	String fileName= FunasrWsClientTest.wavPath;
	String suffix=fileName.split("\\.")[fileName.split("\\.").length-1];
    sendJson(mode, strChunkSize, chunkInterval, wavName, true,suffix);
    File file = new File(FunasrWsClientTest.wavPath);
    ...
    try (FileInputStream fis = new FileInputStream(file)) {
      if (FunasrWsClientTest.wavPath.endsWith(".wav")) {
  ...
  public static void main(String[] args) throws URISyntaxException {
    ...
    FunasrWsClientTest.strChunkSize = chunk_size;
    FunasrWsClientTest.chunkInterval = chunk_interval;
    FunasrWsClientTest.wavPath = wavPath;
    FunasrWsClientTest.mode = strmode;
	FunasrWsClientTest.hotwords=hot;
    System.out.println(
        "serIp="
            + srvIp
            + ",srvPort="
    ...
    class ClientThread implements Runnable {
      ...
      public void run() {
        try {
          ...
          FunasrWsClientTest.sendChunkSize = CHUNK * 2;

          String wsAddress = "ws://" + srvIp + ":" + srvPort;

          FunasrWsClientTest c = new FunasrWsClientTest(new URI(wsAddress));
